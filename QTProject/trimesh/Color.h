...
class Color : public TriVec<3,float> {
public:
	Color()
		{}
	Color(const TriVec<3,float> &v_) : TriVec<3,float>(v_)
		{}
	Color(const TriVec<3,double> &v_) : TriVec<3,float>((float)v_[0], (float)v_[1], (float)v_[2])
		{}
	Color(float r, float g, float b) : TriVec<3,float>(r,g,b)
		{}
	Color(double r, double g, double b) : TriVec<3,float>((float)r, (float)g, (float)b)
		{}
	explicit Color(const float *rgb) : TriVec<3,float>(rgb[0], rgb[1], rgb[2])
		{}
	explicit Color(const double *rgb) : TriVec<3,float>((float)rgb[0], (float)rgb[1], (float)rgb[2])
		{}

	// Implicit conversion from float would be bad, so we have an
	// explicit constructor and an assignment statement.
	explicit Color(float c) : TriVec<3,float>(c,c,c)
		{}
	explicit Color(double c) : TriVec<3,float>((float)c, (float)c, (float)c)
		{}
