...
//************ rename the Vec to TVec by liuxiang,2012/11/30,because it is conflicated with opencv
...
public:
	...
	TriVec() { for (int i = 0; i < D; i++) v[i] = T(0); }
...
#define VEC_UNINITIALIZED ((void *) 0)
	...
	explicit TriVec(const T &x)
		{ for (int i = 0; i < D; i++) v[i] = x; }

	// Constructors for 2-4 arguments
	TriVec(const T &x, const T &y)
		{ VEC_STATIC_CHECK(D == 2); v[0] = x; v[1] = y; }
	TriVec(const T &x, const T &y, const T &z)
		{ VEC_STATIC_CHECK(D == 3); v[0] = x; v[1] = y; v[2] = z; }
	TriVec(const T &x, const T &y, const T &z, const T &w)
	...
	template <class S> explicit TriVec(const S &x)
	...
	// Member operators
	TriVec<D,T> &operator += (const TriVec<D,T> &x)
	{
		for (int i = 0; i < D; i++)
#pragma omp atomic
			v[i] += x[i];
...
#pragma omp atomic
	...
	TriVec<D,T> &min(const TriVec<D,T> &x)
	{
#pragma omp critical
		for (int i = 0; i < D; i++)
			if (x[i] < v[i]) v[i] = x[i];
	...
	TriVec<D,T> &max(const TriVec<D,T> &x)
	{
#pragma omp critical
		for (int i = 0; i < D; i++)
			if (x[i] > v[i]) v[i] = x[i];
...
#pragma omp critical
	...
	TriVec<D,T> apply(T func(T)) const
	{
		TriVec<D,T> result(VEC_UNINITIALIZED);
		for (int i = 0; i < D; i++) result[i] = func(v[i]);
		return result;
	}
	TriVec<D,T> apply(T func(const T&)) const
	{
		TriVec<D,T> result(VEC_UNINITIALIZED);
		for (int i = 0; i < D; i++) result[i] = func(v[i]);
		return result;
	}
	TriVec<D,T> cshift(int n) const
	{
		TriVec<D,T> result(VEC_UNINITIALIZED);
		if (n < 0)
			n = (n % D) + D;
	...
	TriVec<D,T> shift(int n) const
	{
		if (abs(n) >= D)
			return TriVec<D,T>();
		TriVec<D,T> result; // Must be initialized to zero
		int start = n < 0 ? -n : 0;
		int stop = n > 0 ? D - n : D;
...
typedef TriVec<3,float> vec;
typedef TriVec<3,float> point;
typedef TriVec<3,float> vnormal;
typedef TriVec<2,float> vtexture;
typedef TriVec<2,float> vec2;
typedef TriVec<3,float> vec3;
typedef TriVec<4,float> vec4;
typedef TriVec<2,int> ivec2;
typedef TriVec<3,int> ivec3;
typedef TriVec<4,int> ivec4;


// Nonmember operators that take two Vecs
template <int D, class T>
static inline const TriVec<D,T> operator + (const TriVec<D,T> &v1, const TriVec<D,T> &v2)
{
	TriVec<D,T> result(VEC_UNINITIALIZED);
	for (int i = 0; i < D; i++)
		result[i] = v1[i] + v2[i];
...
// Dot product
template <int D, class T>
static inline const T operator ^ (const TriVec<D,T> &v1, const TriVec<D,T> &v2)
{
	T sum = v1[0] * v2[0];
	for (int i = 1; i < D; i++)
		sum += v1[i] * v2[i];
...
template <class T>
static inline const TriVec<3,T> operator % (const TriVec<3,T> &v1, const TriVec<3,T> &v2)
{
	return TriVec<3,T>(v1[1]*v2[2] - v1[2]*v2[1],
			v1[2]*v2[0] - v1[0]*v2[2],
			v1[0]*v2[1] - v1[1]*v2[0]);
}
#define CROSS %
...
template <int D, class T>
static inline bool operator == (const TriVec<D,T> &v1, const TriVec<D,T> &v2)
{
	for (int i = 0; i < D; i++)
		if (v1[i] != v2[i])
			return false;
...
// Unary operators
template <int D, class T>
static inline const TriVec<D,T> &operator + (const TriVec<D,T> &v)
{
	return v;
}
...
template <int D, class T>
static inline bool operator ! (const TriVec<D,T> &v)
{
	return v.empty();
}


// TriVec/scalar operators
template <int D, class T>
static inline const TriVec<D,T> operator * (const T &x, const TriVec<D,T> &v)
{
	TriVec<D,T> result(VEC_UNINITIALIZED);
	for (int i = 0; i < D; i++)
		result[i] = x * v[i];
...
template <int D, class T>
static inline const TriVec<D,T> operator * (const TriVec<D,T> &v, const T &x)
{
	TriVec<D,T> result(VEC_UNINITIALIZED);
	for (int i = 0; i < D; i++)
		result[i] = v[i] * x;
...
// iostream operators
template <int D, class T>
static inline std::ostream &operator << (std::ostream &os, const TriVec<D,T> &v)
...
template <int D, class T>
static inline std::istream &operator >> (std::istream &is, TriVec<D,T> &v)
{
	char c1 = 0, c2 = 0;
...
namespace std {
  template <int D, class T>
  static inline void swap(const TriVec<D,T> &v1, const TriVec<D,T> &v2)
  {
	for (int i = 0; i < D; i++)
		swap(v1[i], v2[i]);
  }
...
template <int D, class T>
static inline T faceforward(const TriVec<D,T> &N, const TriVec<D,T> &I,
			    const TriVec<D,T> &Nref)
{
	return ((Nref DOT I) < T(0)) ? N : -N;
}

template <int D, class T>
static inline T reflect(const TriVec<D,T> &I, const TriVec<D,T> &N)
{
	return I - (T(2) * (N DOT I)) * N;
}

template <int D, class T>
static inline T refract(const TriVec<D,T> &I, const TriVec<D,T> &N,
			const T &eta)
{
...
// Squared length
template <int D, class T>
static inline const T len2(const TriVec<D,T> &v)
{
	T l2 = v[0] * v[0];
	for (int i = 1; i < D; i++)
		l2 += v[i] * v[i];
...
// Length
template <int D, class T>
static inline const T len(const TriVec<D,T> &v)
{
	using namespace std; // For sqrt
	return sqrt(len2(v));
}
...
// Squared distance
template <int D, class T>
static inline const T dist2(const TriVec<D,T> &v1, const TriVec<D,T> &v2)
{
	T d2 = sqr(v2[0]-v1[0]);
	for (int i = 1; i < D; i++)
		d2 += sqr(v2[i]-v1[i]);
...
// Distance
template <int D, class T>
static inline const T dist(const TriVec<D,T> &v1, const TriVec<D,T> &v2)
{
	using namespace std; // For sqrt
...
template <int D, class T>
static inline TriVec<D,T> normalize(TriVec<D,T> &v)
{
	T l = len(v);
	if (unlikely(l <= T(0))) {
		v[0] = T(1);
...
template <int D, class T>
static inline const T angle(const TriVec<D,T> &v1, const TriVec<D,T> &v2)
{
	using namespace std;
...
#define VEC_DECLARE_ONEARG(name) \
 template <int D, class T> \
 static inline TriVec<D,T> name(const TriVec<D,T> &v) \
 { \
	using namespace std; \
	TriVec<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
...
#define VEC_DECLARE_TWOARG(name) \
 template <int D, class T> \
 static inline TriVec<D,T> name(const TriVec<D,T> &v, const T &w) \
 { \
	using namespace std; \
	TriVec<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
 ...
 template <int D, class T> \
 static inline TriVec<D,T> name(const TriVec<D,T> &v, const TriVec<D,T> &w) \
 { \
	using namespace std; \
	TriVec<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
...
#define VEC_DECLARE_THREEARG(name) \
 template <int D, class T> \
 static inline TriVec<D,T> name(const TriVec<D,T> &v, const T &w, const T &x) \
 { \
	using namespace std; \
	TriVec<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
 ...
 template <int D, class T> \
 static inline TriVec<D,T> name(const TriVec<D,T> &v, const TriVec<D,T> &w, const TriVec<D,T> &x) \
 { \
	using namespace std; \
	TriVec<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
...
// Let's do the same...
template <int D, class T>
static inline TriVec<D,T> abs(const TriVec<D,T> &v)
{
	return fabs(v);
}
