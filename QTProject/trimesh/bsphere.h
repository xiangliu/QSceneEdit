...
public:
	...
	T excess(const TriVec<D,T> &p) const;
	void reset();		// generates empty sphere with m=s=0
	bool push(const TriVec<D,T> &p);
	void pop();
};
...
class Miniball {
public:
	typedef typename std::list< TriVec<D,T> >::iterator It;

private:
	std::list< TriVec<D,T> > L;// STL list keeping the points
...
public:
	void check_in(const TriVec<D,T> &p) { L.push_back(p); }
	void check_in(const TriVec<D,T>*p,int size) 
	{
	...
	TriVec<D,T> center() const { return TriVec<D,T>(B.center()); }
	T squared_radius() const { return B.squared_radius(); }
};


template <int D, class T>
T Basis<D,T>::excess(const TriVec<D,T> &p) const
{
	T e = -current_sqr_r;
	for (int k = 0; k < D; k++)
...
template <int D, class T>
bool Basis<D,T>::push(const TriVec<D,T> &p)
{
	int i, j;
