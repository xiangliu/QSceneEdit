...
//added by liuxiang:: 用于只对一个场景进行分类
int NB::classify_OneScene(sparse_feat classifyData, int event_model)
{
	int pred_class; 
	vector<float> pred_score;

	if (event_model == 0) {
		pred_score = predict_logp_bernoulli(classifyData);
	}
	else {
		pred_score = predict_logp_multinomial(classifyData);
	}       
